...
import {
    ...
    GetDashboardQuery, Review_Insert_Input,
...
const AddProductReviewForm = ({ selectCats, selectProducts, onBackAction, onSuccess }: AddProductReviewFormProps) => {
    const {
        handleSubmit,
        control,
        formState: { errors },
    ...
    const [createReview, {error, loading, data}] = useMutation<AddReviewMutation,
        AddReviewMutationVariables>(ADD_REVIEW);
    ...
    const onSubmit =
        (data: any) => {
            ...
            createReview({ variables: { review: reviewInput } })
                .then((data) => {
                    onSuccess();
                    onBackAction();
                });
            ...
            <Controller
                ...
                render={({ field }) =>
                    <Select<SelectProductFieldsFragment>
                        ...
                        getOptionValue={(product: SelectProductFieldsFragment) => product.id.toString()}
                        getOptionLabel={(product: SelectProductFieldsFragment) => product.name}
                    />
                }
            />
